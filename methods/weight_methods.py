...
class PMGDN(WeightMethod):  # use for log_pmgd-gamma0.001-42_3&4&240.log
    def __init__(
        self, n_tasks, device: torch.device, params="shared", normalization="none", backward_counter=0, pmgd_x=11, task_weights: Union[List[float], torch.Tensor] = None,
        gamma: float = 1e-5, w_lr: float = 0.025, max_norm: float = 1.0, alpha=0.9
    ):
        ...
        if task_weights is None:
            task_weights = torch.ones((n_tasks,), device=device)
        ...
        self.max_norm = max_norm
        self.prev_loss = None  # Initialize prev_loss to None
        self.prev_weights = None  # Initialize prev_w to None
        self.alpha = alpha
    ...
    def backward(
        ...
        **kwargs,
        ...
        if self.backward_counter % self.pmgd_x == 0:
            loss, extra_outputs = self.get_weighted_loss_min_norm(
                losses, shared_parameters, last_shared_parameters, representation)
        else:
            loss, extra_outputs = self.get_weighted_loss_task_weights(losses)

        self.backward_counter += 1

        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        # Check if 'weights' are in extra_outputs and update them
    ...
    # with FAMO-like modifications
    def get_weighted_loss_min_norm(self, losses, shared_parameters=None, last_shared_parameters=None, representation=None):
        ...
        for i, loss in enumerate(losses):
            grads[i] = torch.autograd.grad(
                (loss + 1e-8).log(), params, retain_graph=True, allow_unused=True)
            grads[i] = [g.flatten() for g in grads[i] if g is not None]

        gn = gradient_normalizers(grads, losses.tolist(), self.normalization)
        for t in grads:
        ...
        # Apply FAMO-like modifications
        self.prev_loss = losses
        z = F.softmax(self.w, -1)
        D = losses - self.min_losses + 1e-8
        c = (z / D).sum().detach()
        sol_tensor = torch.tensor(sol, dtype=losses.dtype, device=self.device)
        weighted_loss = (D.log() * z / c * sol_tensor).sum()

        return weighted_loss, dict(weights=sol_tensor)
    # without FAMO-like modifications
    # def get_weighted_loss_min_norm(self, losses, shared_parameters=None, last_shared_parameters=None, representation=None):
    #     params = dict(rep=representation, shared=shared_parameters,
    #                   last=last_shared_parameters)[self.params]
    ...
    #     c = sum([sol[i] / (losses[i] + 1e-8).item() for i in range(len(sol))])
    #     sol = sol / c

    #     sol_tensor = torch.tensor(sol, dtype=losses.dtype, device=self.device)
    #     weighted_loss = torch.sum((losses + 1e-8).log() * sol_tensor)
    #     return weighted_loss, dict(weights=sol_tensor)

    def get_weighted_loss_task_weights(self, losses):
        self.prev_loss = losses  # Update prev_loss with the current losses
        z = F.softmax(self.w, -1)
        D = losses - self.min_losses + 1e-8
        c = (z / D).sum().detach()
        loss = (D.log() * z / c).sum()
    ...
    def update(self, curr_loss):
        if self.prev_loss is None:
            # If prev_loss is None, skip the update
            return
...
class PMGDNLog(WeightMethod):
    def __init__(
        self, n_tasks, device: torch.device, params="shared", normalization="none", backward_counter=0, pmgd_x=11, task_weights: Union[List[float], torch.Tensor] = None,
        gamma: float = 1e-5, w_lr: float = 0.025, max_norm: float = 1.0, alpha=0.5
    ):
        super().__init__(n_tasks, device=device)
        self.solver = MinNormSolver()
        assert params in ["shared", "last", "rep"]
...
class PMGDLog1(WeightMethod):  # log version for arrya-ones PMGD
    def __init__(
        self, n_tasks, device: torch.device, params="shared", normalization="none", backward_counter=0, pmgd_x=8, task_weights: Union[List[float], torch.Tensor] = None, alpha=0.9
    ):
        super().__init__(n_tasks, device=device)
        self.solver = MinNormSolver()
        assert params in ["shared", "last", "rep"]
    ...
    def get_weighted_loss_task_weights(self, losses):
        weighted_loss = torch.dot(losses, self.task_weights)
        return weighted_loss, dict(weights=self.task_weights)


class PMGD(WeightMethod):  # Just use sol in the backward, it should be used for our theory
    def __init__(
        self, n_tasks, device: torch.device, params="shared", normalization="none", pmgd_x=4, sol=None, alpha=0.9
    ):
        super().__init__(n_tasks, device=device)
        self.solver = MinNormSolver()
        assert params in ["shared", "last", "rep"]
...
class LOG_MGDA(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        # sol = sol * self.n_tasks  # make sure it sums to self.n_tasks
        c = sum([sol[i] / (losses[i] + 1e-8).detach()
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for param in shared_parameters:
            grad_dims.append(param.data.numel())
        grads = [torch.Tensor(
            sum(grad_dims), self.n_tasks).to(self.device)] * 3
        ...
        for i in range(self.n_tasks * 3):
            if i < self.n_tasks * 3 - 1:
                ...
                losses[i].backward()
            self.grad2vec(shared_parameters, grads[int(
                i / self.n_tasks)], grad_dims, int(i % self.n_tasks))
            # multi_task_model.zero_grad_shared_modules()
            for p in shared_parameters:
                p.grad = None

        g, GTG = self.sdmgrad(
            self.w, grads, lamda=self.lamda, niter=self.niter)
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return GTG
