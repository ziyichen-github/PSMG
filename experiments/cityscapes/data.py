...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, label, depth):
        ...
        depth_ = F.interpolate(
            depth[None, :, i: i + h, j: j +
                  w], size=(height, width), mode="nearest"
        ).squeeze(0)
...
class Cityscapes(Dataset):
    ...
    def __init__(self, root, train=True, augmentation=False):
        ...
        # read the data file
        if train:
            self.data_path = "/home/mx6835/Academic/dataset4all/FAMO/cityscapes2" + '/train'
        else:
            self.data_path = "/home/mx6835/Academic/dataset4all/FAMO/cityscapes2" + '/val'

        # calculate data length
        self.data_len = len(fnmatch.filter(
            os.listdir(self.data_path + '/image'), '*.npy'))
